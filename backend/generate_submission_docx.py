...
def create_submission_document(output_path: str):
    ...
    for section in doc.sections:
        ...
        r_ft = p_ft.add_run(
            "Darukaa.Earth — Full-Stack Developer Hackathon Submission Report  |  Confidential"
        )
    ...
    run_sub = p_sub.add_run(
        "Official Technical Architecture, Product Specification & Submission Deliverables Report"
    )
    ...
    t1_data = [
        ...
        (
            ...
            "http://127.0.0.1:8000/api/v1/docs (FastAPI + OpenAPI Spec)",
        ),
        ("Pre-Seeded Admin Credentials", "Email: admin@darukaa.earth  |  Password: AdminPass123!"),
        (
            "Reviewers Invited / Granted Access",
    ...
    gbif_features = [
        (
            "Spatial Geometry Querying",
            "Converts PostGIS polygon coordinates into spatial bounding boxes and WKT polygons to query GBIF occurrence endpoints (/occurrence/search).",
        ),
        (
            "IUCN Red List Matrix",
            "Categorizes observed species according to official IUCN threat levels: Critically Endangered (CR), Endangered (EN), Vulnerable (VU), Near Threatened (NT), and Least Concern (LC).",
        ),
        (
            "Shannon-Wiener Biodiversity Index (H')",
            "Computes the ecological entropy index using H' = -SUM(p_i * ln(p_i)), normalized to a 0–100 Biodiversity Health Score.",
        ),
        (
            "Kingdom & Taxonomic Breakdown",
            "Provides interactive distribution breakdown across Animalia, Plantae, Fungi, and major biological classes.",
        ),
        (
            "Species Observation Catalog",
            "Interactive UI gallery displaying common vernacular names, scientific binomial names, observation dates, high-resolution media imagery, and direct links to official GBIF occurrence records.",
        ),
    ]
    ...
    eval_matrix = [
        (
            ...
            "20 unit and integration tests covering Authentication, Project CRUD, PostGIS Site Geometries, Geodesic Area Computations, MRV Analytics, and GBIF Telemetry (20/20 Passing).",
        ),
        (
            "Pre-Commit Hooks (Husky & lint-staged)",
            "Automated execution of Prettier, ESLint, and Ruff formatting/linting before commits.",
        ),
        (
            "GitHub Actions CI (.github/workflows/ci.yml)",
            "Continuous Integration matrix verifying Python test suites, TypeScript compilation, and Vite production bundle builds on all pull requests.",
        ),
        (
            "Dual-Engine Spatial Fallback",
            "Engineered with native PostGIS for production enterprise scalability and an intelligent zero-config SQLite spatial fallback for instant local evaluation without requiring external database setup.",
        ),
    ]
...
if __name__ == "__main__":
    out = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "Darukaa_Earth_Hackathon_Submission.docx")
    )
